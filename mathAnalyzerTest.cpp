...
{
    EXPECT_THROW(MathAnalyzer::ParseExpression("(6.0*2.0)-4.0() / 2"), std::invalid_argument);
}
...
{
    EXPECT_THROW(MathAnalyzer::ParseExpression("(2+((6.0-1.0))"), std::invalid_argument);
}
...
{
    EXPECT_NEAR(-26.0, MathAnalyzer::ParseExpression("-( 6.0 - 1.0 ) * 4 - 2 * 3"), 0.01);
}


TEST(MathAnalyzerParsing, parseDevisionByZeroCase)
{
    EXPECT_THROW(MathAnalyzer::ParseExpression("(2+5) / (9 - 3 * 3)"), std::runtime_error);
}
